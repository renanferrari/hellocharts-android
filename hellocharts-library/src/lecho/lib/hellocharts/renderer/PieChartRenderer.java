...
/**
 * Default renderer for PieChart. PieChart doesn't use viewport concept so it a little different
 * than others chart
 * types.
 */
public class PieChartRenderer extends AbstractChartRenderer {
  ...
  // Translucent circle related attributes
  private boolean hasTranslucentCircle;
  private float translucentCircleScale;
  private Paint translucentCirclePaint = new Paint();
  // Text1
  ...
  public PieChartRenderer(Context context, Chart chart, PieChartDataProvider dataProvider) {
    ...
    translucentCirclePaint.setAntiAlias(true);
    translucentCirclePaint.setStyle(Paint.Style.FILL);
    translucentCirclePaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_ATOP));
  ...
  @Override public void onChartSizeChanged() {
    calculateCircleOval();
  ...
  @Override public void onChartDataChanged() {
    ...
    hasTranslucentCircle = data.hasTranslucentCircle();
    translucentCircleScale = data.getTranslucentCircleScale();
    translucentCirclePaint.setColor(data.getTranslucentCircleColor());
    if (translucentCirclePaint.getAlpha() == 255) {
      translucentCirclePaint.setAlpha(Math.round(data.getTranslucentCircleOpacity() * 255));
    }
    if (null != data.getCenterText1Typeface()) {
      centerCircleText1Paint.setTypeface(data.getCenterText1Typeface());
    }
    centerCircleText1Paint.setTextSize(
        ChartUtils.sp2px(scaledDensity, data.getCenterText1FontSize()));
    ...
    centerCircleText2Paint.setTextSize(
        ChartUtils.sp2px(scaledDensity, data.getCenterText2FontSize()));
  ...
  @Override public void onChartViewportChanged() {
  ...
  @Override public void draw(Canvas canvas) {
    ...
    if (hasCenterCircle) {
      ...
      if (hasTranslucentCircle) {
        drawTranslucentCircle(drawCanvas);
      }

      // Text must be drawn for last, otherwise the translucent circle would overlap it
      drawCenterText(drawCanvas);
    }
  ...
  @Override public void drawUnclipped(Canvas canvas) {
  }

  @Override public boolean checkTouch(float touchX, float touchY) {
    selectedValue.clear();
    final PieChartData data = dataProvider.getPieChartData();
    ...
    if (data.hasCenterCircle() &&
        sliceVector.length() < circleRadius * data.getCenterCircleScale()) {
      return false;
    }
    ...
    final float touchAngle = (pointToAngle(touchX, touchY, centerX, centerY) - rotation + 360f) %
                             360f;
  ...
  /**
   * Draw translucent circle if {@link PieChartData#hasCenterCircle()} and
   * {@link PieChartData#hasTranslucentCircle()} are set true.
   */
  private void drawTranslucentCircle(Canvas canvas) {
    final PieChartData data = dataProvider.getPieChartData();
    final float circleRadius = originCircleOval.width() / 2f;
    final float centerRadius = circleRadius * data.getCenterCircleScale();
    final float translucentRadius = centerRadius + ((circleRadius - centerRadius) *
                                                    data.getTranslucentCircleScale());
    final float centerX = originCircleOval.centerX();
    final float centerY = originCircleOval.centerY();

    canvas.drawCircle(centerX, centerY, translucentRadius, translucentCirclePaint);
  }

  /**
   * Draw center text if {@link PieChartData#hasCenterCircle()} is set true.
   */
  private void drawCenterText(Canvas canvas) {
    ...
    if (!TextUtils.isEmpty(data.getCenterText1())) {
      ...
      if (!TextUtils.isEmpty(data.getCenterText2())) {
        ...
        canvas.drawText(data.getCenterText1(), centerX, centerY - text1Height * 0.2f,
                        centerCircleText1Paint);
        canvas.drawText(data.getCenterText2(), centerX, centerY + text2Height,
                        centerCircleText2Paint);
      } else {
        canvas.drawText(data.getCenterText1(), centerX, centerY + text1Height / 4,
                        centerCircleText1Paint);
      }
    }
  }
  ...
  /**
   * Draw all slices for this PieChart, if mode == {@link #MODE_HIGHLIGHT} currently selected slices
   * will be redrawn
   * and
   * highlighted.
   */
  ...
  private void drawSeparationLines(Canvas canvas) {
    ...
    for (SliceValue sliceValue : data.getValues()) {
      ...
      canvas.drawLine(originCircleOval.centerX(), originCircleOval.centerY(), x1, y1,
  ...
  /**
   * Method draws single slice from lastAngle to lastAngle+angle, if mode = {@link #MODE_HIGHLIGHT}
   * slice will be
   * darken
   * and will have bigger radius.
   */
  private void drawSlice(Canvas canvas, SliceValue sliceValue, float lastAngle, float angle,
                         int mode) {
  ...
  private void drawLabel(Canvas canvas, SliceValue sliceValue, float lastAngle, float angle) {
    ...
    final float labelWidth = labelPaint.measureText(labelBuffer, labelBuffer.length - numChars,
                                                    numChars);
  ...
  /**
   * Viewport is not really important for PieChart, this kind of chart doesn't relay on viewport but
   * uses pixels
  ...
  /**
   * Returns SliceValue that is under given angle, selectedValue (if not null) will be hold slice
   * index.
   */
  ...
  /**
   * Set how much of view area should be taken by chart circle. Value should be between 0 and 1.
   * Default is 1 so
